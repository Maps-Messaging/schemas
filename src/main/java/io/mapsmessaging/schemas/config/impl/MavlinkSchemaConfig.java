...
@Schema(description = "MAVLink Schema Configuration")
public class MavlinkSchemaConfig extends SchemaConfig {
  ...
  public MavlinkSchemaConfig() {
    super("mavlink");
  }

  protected MavlinkSchemaConfig(SchemaConfig config) {
    super(config);
  }
  ...
  public String pack() throws IOException {
    ...
    if ((dialect == null || dialect.isBlank()) && (xml == null || xml.isBlank())) {
      throw new IOException("MAVLink config requires either 'dialect' or 'dialectXml'");
    }

    if (dialect != null && !dialect.isBlank() && xml != null && !xml.isBlank()) {
      throw new IOException("MAVLink config must specify only one of 'dialect' or 'dialectXml'");
    }
