/*
 * FieldSpecification — STRICT CBC KEYS ONLY
 *
 * Valid keys per CBC doc:
 *  name (M)          String
 *  type (M)          String
 *  size (C)          Number  (not for struct; fixed bits/bytes/elements if fixed=true, else max length)
 *  optional (O)      Boolean
 *  fixed (O)         Boolean  (default true)
 *  enum (C)          Object   {"0":"FOO","1":"BAR"} for enum/bitmask/bitmaskarray
 *  fields (C)        Field[]  required for struct/array
 *  description (O)   String
 *  decalc (O)        String   e.g. "v/1000"
 *  encalc (O)        String   inverse of decalc
 *  min (O)           Int
 *  max (O)           Int
 *
 * No extra/internal keys are serialized. Parser accepts ONLY these keys.
 */
